...
- \`webhooks:write\`: Provision or configure webhooks
  ...
  async getToken(@Req() req: any, @Body() body: V3TokenRequestDto) {
    ...
    // Find Organization (as the M2M connection) by clientId
    ...
    if (org) {
      if (!org.clientSecret) {
        throw new UnauthorizedException('Invalid client credentials: No client secret configured.');
      }
      ...
      // Issue token for Machine-to-Machine context (Organization)
